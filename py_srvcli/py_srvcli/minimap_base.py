...
class MinimalSubscriber(Node):
    def __init__(self):
        ...
        qos_profile = QoSProfile(
            reliability=ReliabilityPolicy.BEST_EFFORT,  # 메시지 손실 허용
            history=HistoryPolicy.KEEP_LAST,  # 최신 메시지만 유지
            depth=5,  # 메시지 큐 크기 설정
            durability=QoSDurabilityPolicy.VOLATILE,  # 과거 데이터 무시
        )
...
# def run_ros_nodes():
#     """ROS 2 Subscriber와 Operator 노드를 동시에 실행"""
#     rclpy.init()
...
def main():
    ...
    app.run(debug=True, host="0.0.0.0", port=5000)  # Flask 서버 실행


# if __name__ == "__main__":
#     thread1 = threading.Thread(target=ros2_spin)
#     thread2 = threading.Thread(target=start_ros_node)
